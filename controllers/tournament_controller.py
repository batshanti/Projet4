...
class Tournament_controller:
    ...
    def first(self):
        ...
        self.tournament.players_object =  Tournament_controller.update_score(self.tournament.players_object, round_1_score)
    ...
    def round(self):
        ...
        # sort_score_players = sorted(self.tournament.players, key=lambda t: (t['score'],t['rank']), reverse=True)
        start_time = time.strftime("%d/%m/%Y")+" "+time.strftime("%Hh%Mm%Ss")
        self.tournament.sort_player_score()
        round_score = self.play_tournament_view.round(self.tournament.players_object, self.tournament.name, round_name)    
        round = Round(round_name, start_time)
        self.tournament.rounds = self.tournament.rounds + [round.save(round_score)]
        self.tournament.players_object =  Tournament_controller.update_score(self.tournament.players_object, round_score)
        self.tournament.save_tournament(1)
        next = -1
        while next != 0 and next != 1:
            next = self.play_tournament_view.next_round()
